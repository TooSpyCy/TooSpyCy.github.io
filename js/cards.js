...
async function openPack() {
  ...
  // Met à jour le profil local avec le nouveau nbrpacks retourné par le serveur
  if (currentProfile) currentProfile.nbrpacks = result.nbrpacks;
  updatePackCount();

  return result.cards; // même format que l'ancien drawn : [{ carte, version }]
}
...
function probabilityLabel(carteProba, versionMult) {
  const pct = (parseFloat(carteProba) * parseFloat(versionMult) * 100).toFixed(
    2,
  );
  return `${pct}%`;
}
